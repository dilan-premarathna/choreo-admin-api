...
@test:Mock {
    moduleName: "choreo_admin_api.auth",
    functionName: "authorize"
}
test:MockFunction mockAuthorize = new ();

@test:Mock {
    moduleName: "choreo_admin_api.auth",
    functionName: "authenticate"
}
test:MockFunction mockAuthenticate = new ();

@test:Config {}
function testGetSubscriptionOfOrgService() returns error? {
    test:when(mockAuthorize).thenReturn(true);
    test:when(mockAuthenticate).thenReturn(mockJWTPayload);
    test:when(mockGetOrgSubsctiptionDetails).thenReturn(mockSubscription);
    http:Response response = check testClient->get("/subscription/12345", {"x-forwarded-authorization": mockJWT});
...
function testUpdateOnTierService() returns error? {
    ...
    http:Response response = check testClient->put("/subscription/123455", {
        "stepQuota": "5000",
        "tierName": "Enterprise"
    }, {"x-forwarded-authorization": mockJWT});
    test:assertEquals(response.statusCode, http:STATUS_OK);
}
